<div class="msg">

    <div class="<?=(($scroll_type!='')?$scroll_type:'screen-scroll-sm');?> msg-screen-scroll auto-scroll-bottom pt-20">


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-general">
            <div>
                May 27, 2019
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>

        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-general">
            <div>
                May 27, 2019
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>

        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-general">
            <div>
                May 27, 2019
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>

        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-outbox">
            <div>
                <div>
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                    The actual message here the actual message here the actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    The actual message here
                    The actual message here
                    The actual message here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


        <div class="msg-inbox">
            <div>
                <div>
                    last actual message is here
                </div>
                <div>
                    11:05am
                </div>
            </div>
        </div>


    </div>


    <div class="msg-new box-shadow-mini">
        <div class="p-sm-15-5 pb-10 bt-1-ddd bb-1-ddd bg-eee">
            <div>
                <div class="form-group">
                    <textarea class="form-control resize-none" style="height: 45px;" placeholder="Type a message..." required></textarea>
                </div>
            </div>
            <div class="floated-content">
                <button type="submit" class="btn btn-blue btn-sm ml-10 pull-right">
                    <span class="fa fa-paper-plane"></span>
                    Send
                </button>
                <button type="button" class="btn btn-transparent-black btn-sm pull-right" data-toggle="collapse" data-target="#add-attachment">
                    <span class="fa fa-paperclip icon-333"></span>
                    Add attachment
                    <span class="fa fa-angle-down icon-333"></span>
                </button>
            </div>
            <div id="add-attachment" class="collapse">
                <div class="mt-10 b-1-ddd p-20-10 pt-5">
                    <div class="row">
                        <div class="col-sm-6 mt-10">
                            <input type="file" class="form-control">
                        </div>
                        <div class="col-sm-6 mt-10">
                            <input type="file" class="form-control">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    
</div>